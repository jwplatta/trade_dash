"""OI z-score computation: bucket-based historical comparison."""
...
def build_oi_bucket_stats(
    ...
    Returns:
        ...
        columns oi_mean, oi_std, count.
    """
    pieces: list[pd.DataFrame] = []
    for df, sample_dt in zip(frames, sample_datetimes, strict=False):
        needed = {"strike", "open_interest", "underlying_price", "expiration_date", "contract_type"}
        ...
        pieces.append(row[["contract_type", "dte_bucket", "moneyness_bucket", "open_interest"]])

    if not pieces:
        return pd.DataFrame(columns=["oi_mean", "oi_std", "count"])
    ...
    stats = (
        ...
        .agg(oi_mean="mean", oi_std="std", count="count")
        .reset_index()
...
def compute_oi_zscore_matrix(
    ...
    """Compute OI z-scores for each (expiry, strike) using bucket stats.

    Args:
        current_snapshots: Mapping of expiry → latest snapshot DataFrame.
        bucket_stats: Output of build_oi_bucket_stats().
        spot: Current underlying price.
        contract_type: "CALL", "PUT", or "OTM".
        now: Current UTC datetime (used to compute DTE and filter near-expiry 0DTE).
        dte_edges: Must match those used in build_oi_bucket_stats().
        moneyness_edges: Must match those used in build_oi_bucket_stats().
    ...
    for expiry, df in current_snapshots.items():
        ...
        options = options.dropna(subset=["open_interest", "strike"])
        ...
        for _, opt in options.iterrows():
            ...
            z = float("nan") if cnt < 3 or std == 0 or math.isnan(std) else (oi - mean) / std
