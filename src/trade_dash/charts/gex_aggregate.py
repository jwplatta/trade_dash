...
def build_gex_aggregate_chart(
    ...
    fig.add_trace(go.Bar(
        x=strike_gex["strike"], y=strike_gex["net_gex"],
        name="Net GEX by Strike", marker_color=colors, opacity=0.7,
    ))
    fig.add_trace(go.Scatter(
        x=price_gex["price"], y=price_gex["net_gex"] * scale,
        name="Net GEX by Price (scaled)", line={"color": "yellow", "width": 2},
    ))
    fig.add_hline(y=0, line_dash="solid", line_color="white", line_width=0.5)
    fig.add_vline(
        x=spot, line_dash="dash", line_color="white",
        annotation_text=f"Spot {spot:.0f}", annotation_position="top right",
    )
    if zgl is not None:
        fig.add_vline(
            x=zgl, line_dash="dot", line_color="yellow",
            annotation_text=f"ZGL {zgl:.0f}", annotation_position="top left",
        )
    fig.update_layout(
        title=title,
