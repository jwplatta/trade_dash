...
def build_vix_term_chart(
    ...
    fig.add_trace(go.Scatter(
        x=vix["datetime"], y=vix["close"],
        name="VIX (30D)", line={"color": "red", "width": 2},
    ))
    fig.add_trace(go.Scatter(
        x=vix9d["datetime"], y=vix9d["close"],
        name="VIX9D", line={"color": "orange", "width": 2},
    ))
    if vix1d is not None:
        fig.add_trace(go.Scatter(
            x=vix1d["datetime"], y=vix1d["close"],
            name="VIX1D", line={"color": "yellow", "width": 2},
        ))
    fig.update_layout(
        title=title,
